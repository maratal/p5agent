...
apps.append({
    ...
    # What an update needs to rebuild the app the way this install did.
    "app-type": req.get("app-type", ""),
    "app-cmd": req.get("app-cmd", ""),
    "demo": str(req.get("demo", "")).lower() in ("true", "1", "yes"),
    "source": (req.get("path") or "").strip().strip("/"),
})
